...
std::mutex Fourier_graph_Transfer; 								// Mutual Exclusion for RX transfer Sync between Threads
...
// ------------------------------ Variables Locales ------------------------------
    ...
    std::complex<float> mean;									// Eliminar el componente DC
    float max_abs = 0;											// Maximo valor absoluto de la señal para normalizar
...
// ----------------------------------------------- Threads -------------------------------------------------------------------
	...
	std::thread thread_raw_fft(Fourier_Raw_Thread, &Fourier_Raw_Data);	// Hilo de ejecucion para la FFT
...
// ------------------------------------------------ Normalizacion -----------------------------------
    ...
    // Calcular la media de las componentes real e imaginaria
	for(int i = 0; i < Raw_Data.size(); i++){
        mean+= (Raw_Data[i]-mean)/static_cast<float>(i+1);
    }
    // Eliminar el componente DC con la media y obtener el valor maximo de su magnitud
	for(int i = 0; i < Raw_Data.size(); i++){
		Raw_Data[i] -= mean+static_cast<float>(1/255.0);
        if(max_abs<std::abs(Raw_Data[i])){
            max_abs = std::abs(Raw_Data[i]);
        }
    }
    // Normalizar los datos a traves del valor maximo de su magnitud
    for(int i = 0; i < Raw_Data.size(); i++){
		Raw_Data[i] /= max_abs; 
    }
...
// ------------------------------------------------ SFML - Ventana Grafica ---------------------------------------------------------------
 ...
 // ------------------------------------------------- Creacion del Hilo para graficar y ciclo de ejecucion del Hilo FFT -----------------
   
    std::thread thread_graph_raw_fourier(Fourier_Raw_Graph_Thread, &fft_raw_window, &vertices_buffer);	// Hilo de ejecucion para la FFT y graficarla

	while(do_exit == false){	// Hilo de ejecucion hasta que el programa se cierre

// ----------------------------------------------- Control de eventos de la ventana grafica ---------------------------------------------
 ...
 // ------------------------------------------------ Calculo de la FFT y copia de los datos al Vertex Buffer para graficarlos -----------
	...
	if(!Fourier_Data->empty()){	// Si existen datos en el buffer de la FFT
		fftwf_execute(p);		// Ejecuta la FFT
		Fourier_Data->clear();	// Limpia el buffer de datos de la FFT para recibir nuevos datos
	}

	for (uint32_t i = 0; i < samples; i++) { 				// Calcula la magnitud de la FFT en escala logaritmica y acomoda los datos en sus respectivas partes positivas y negativas
		if(i<samples/2.0){
			magnitude[i]=(20*log10(std::hypot(out[samples/2+i][0],out[samples/2+i][1]))+log_scale_offset)/(log_span_factor/2);
		}else{
			magnitude[i]=(20*log10(std::hypot(out[i-samples/2][0],out[i-samples/2][1]))+log_scale_offset)/(log_span_factor/2);
		}
	}

    std::lock_guard<std::mutex> lock(Fourier_graph_Transfer); 							// Bloqueo para poder transferir el Buffer sin interrupciones
    vertices_buffer.resize(samples);

	for (uint32_t i = 0; i < samples; i++) {				// Recorrer todo el buffer 
		// Guarda los datos de la magnitud de la FFT en un buffer temporal de tipo float cuya finalidad sera tranasferir los datos a otro vector con Move Semantics
		vertices_buffer[i]=sf::Vertex(sf::Vector2f(i*2.0/(samples-1)-1,magnitude[i]+1e-10), sf::Color::White);
	}
...
void Fourier_Raw_Graph_Thread(sf::RenderWindow *window, std::vector<sf::Vertex> *Vertices){
	...
	// Ciclo de ejecucion de la ventana grafica
    while(window->isOpen() && do_exit == false){
    
    // Mientras el buffer de transferencia no este vacio, transfiere los datos con Move Semantics
	if(!Vertices->empty()){				
        {
    	std::lock_guard<std::mutex> lock(Fourier_graph_Transfer); 					// Bloqueo para usar el buffer de numeros complejos
		vertices_buffer = std::move(*Vertices);
        }
	...
	// Dibuja los datos de la FFT
    glVertexPointer(2, GL_FLOAT, sizeof(sf::Vertex), &vertices_buffer[0].position);
    glDrawArrays(GL_LINE_STRIP, 0, vertices_buffer.size());
