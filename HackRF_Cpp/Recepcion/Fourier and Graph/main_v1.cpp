...
const unsigned __int32 sample_rate = DEFAULT_SAMPLE_RATE_HZ/10;	// Sample Rate: 2MHz - 20MHz in Hz
const unsigned __int8 lna_gain = 32;							// Low Noise Amplifier (LNA): 0-40dB, 8dB steps
const unsigned __int8 vga_gain = 24;								// Variable Gain Amplifier (VGA): 0-62dB, 2dB steps
...
void Fourier_Raw_Thread(std::vector<std::complex<float>> *Fourier_Data, sf::RenderWindow *window){
	...
	float log_scale_offset = 0;		
	float log_span_factor = 200;																						// Vector para almacenar los puntos (vertices) del plano cartesiano normalizado
